...
mesh_all <- make_mesh(dall, c("X", "Y"), n_knots = 220) # cutoff?
...
fit_the_models <- any(!file.exists(files))
```

```{r fit1, results='hide', eval=fit_the_models}
if (us_version) {
  .spatiotemporal = "off"
} else {
  .spatiotemporal = "off"
...
```{r saverds, eval=fit_the_models, echo=FALSE}
saveRDS(fit_nb2, files[1])
saveRDS(fit_dg, files[2])
saveRDS(fit_dl, files[3])
```

```{r readrds, eval=!fit_the_models, echo=FALSE}
